...
describe('At Month-DayTHour:Minute', function() {
    ...
    before(function() {
        ...
        now = moment(timestamp, 'x').minute(min).hour(hour).date(day).month(month - 1);
    });
    ...
    describe('write', function() {
        it('task added', function () {
            ...
            return doTracking(config, spyOutput, ['-t', `${month}-${day}T${hour}:${min}`, task])
            .then(() => {
    ...
    describe('read', function() {
        ...
        it('read date', function () {
            spyOutput.reset();
            return wait = doTracking(config, spyOutput, ['-p', '-t', `${month}-${day}T${hour}:${min}`]).then(() => {
